...
export class Search extends Workers {
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    this.bot.logger.warn(
                        isMobile,
                        'SEARCH-BING',
                        `Navigation failed (${attempt}/${maxRetries}), retrying: ${errorMsg}`
                    )
                    ...
                    this.bot.logger.error(
                        isMobile,
                        'SEARCH-BING',
                        `Navigation failed after ${maxRetries} attempts: ${errorMsg}`
                    )
                    throw error
                }
            }
    ...
    public async doSearch(_data: DashboardData, page: Page, isMobile: boolean): Promise<number> {
        ...
        try {
            ...
            const sourceOrder: QueryEngine[] = (['gemini', ...configuredSources] as QueryEngine[]).filter(
            ...
            this.bot.logger.debug(isMobile, 'SEARCH-BING', `First 3 queries: ${queries.slice(0, 3).map(q => `"${q}"`).join(', ')}`)

            // Go to bing with retry
            ...
            this.bot.logger.debug(isMobile, 'SEARCH-BING', `Starting main search loop | queries=${queries.length} | missingPointsTotal=${missingPointsTotal}`)

            for (let i = 0; i < queries.length; i++) {
                const query = queries[i] as string

                this.bot.logger.debug(isMobile, 'SEARCH-BING', `Search ${i + 1}/${queries.length} | query="${query.substring(0, 50)}..." | current missing=${missingPointsTotal}`)
                ...
                this.bot.logger.debug(isMobile, 'SEARCH-BING', `Search ${i + 1} result | gained=${gainedPoints} | new missing=${newMissingPointsTotal}`)
            ...
            this.bot.logger.info(isMobile, 'SEARCH-BING', `Main search loop completed | queries processed=${queries.length} | final missing points=${missingPointsTotal}`)
    ...
    private async bingSearch(searchPage: Page, query: string, isMobile: boolean) {
        ...
        if (this.searchCount % refreshThreshold === 0) {
            ...
            await this.gotoWithRetry(searchPage, this.bingHome, isMobile)
        ...
        for (let i = 0; i < maxAttempts; i++) {
            try {
                ...
                if (isMobile) {
                    ...
                    const paramOptions = [
                        'PC=U316&FORM=CHROMN',
                        'PC=esb004&FORM=ANWSB4',
                        'PC=U531&FORM=ANNTA1'
                    ]
                    const randomIndex = Math.floor(Math.random() * paramOptions.length)
                    const params = paramOptions[randomIndex]

                    const searchUrl = `https://www.bing.com/search?q=${encodedQuery}&${params}`
                ...
                const actualUrl = searchPage.url()
                this.bot.logger.debug(
                    isMobile,
                    'SEARCH-BING',
                    `Submitted query to Bing | attempt=${i + 1}/${maxAttempts} | query="${query}" | actualUrl="${actualUrl}"`
                )
    ...
    private async clickRandomLink(page: Page, isMobile: boolean) {
        try {
            ...
            if (isMobile) {
                await this.gotoWithRetry(page, searchPageUrl, isMobile)
