...
export class QueryCore {
    ...
    async queryManager(
        options: {
            shuffle?: boolean
            sourceOrder?: (QueryEngine | 'gemini')[]
            related?: boolean
            langCode?: string
            geoLocale?: string
        } = {}
        ...
        const {
            shuffle = false,
            sourceOrder = ['gemini', 'google', 'wikipedia', 'reddit', 'local'],
            related = true,
            langCode = 'en',
            geoLocale = 'US'
        } = options
        ...
        try {
            ...
            > = {
                ...
                gemini: async () => {
                    ...
                    const topics = await geminiEngine.generateSearchQueries().catch(() => [])
            ...
            for (const source of sourceOrder) {
                ...
                this.bot.logger.debug(this.bot.isMobile, 'QUERY-MANAGER', `Trying source: ${source}`)
                const topics = await Promise.resolve(handler())
                this.bot.logger.debug(this.bot.isMobile, 'QUERY-MANAGER', `Source ${source} returned ${topics.length} topics`)

                if (topics.length) {
                    topicLists.push(topics)
                    // If this is Gemini and it worked, prioritize it by breaking
                    if (source === 'gemini' && topics.length >= 100) {
                        this.bot.logger.info(this.bot.isMobile, 'QUERY-MANAGER', `Gemini returned ${topics.length} queries, using only Gemini`)
                        break
                    }
                }
            }
