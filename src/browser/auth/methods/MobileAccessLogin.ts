...
export class MobileAccessLogin {
    ...
    async get(email: string): Promise<string> {
        try {
            ...
            let code = ''
            let lastUrl = ''
            let passkeyDismissCount = 0
            const maxPasskeyDismiss = 5
            ...
            while (Date.now() - start < this.maxTimeout) {
                ...
                try {
                    ...
                    // Check for passkey interrupt and dismiss it
                    if (url.hostname === 'account.live.com' && url.pathname === '/interrupt/passkey/enroll') {
                        if (passkeyDismissCount >= maxPasskeyDismiss) {
                            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-APP', `Passkey dismissed ${maxPasskeyDismiss} times, skipping OAuth`)
                            return ''
                        }
                        
                        passkeyDismissCount++
                        this.bot.logger.info(this.bot.isMobile, 'LOGIN-APP', `Passkey interrupt detected (${passkeyDismissCount}/${maxPasskeyDismiss}), dismissing`)
                        
                        // Wait for page to fully load
                        await this.page.waitForLoadState('networkidle', { timeout: 5000 }).catch(() => {})
                        
                        const secondaryBtn = 'button[data-testid="secondaryButton"]'
                        await this.page.waitForSelector(secondaryBtn, { state: 'visible', timeout: 5000 }).catch(() => {})
                        const btn = await this.page.$(secondaryBtn).catch(() => null)
                        if (btn) {
