...
export default class BrowserUtils {
    ...
    async tryDismissPasskey(page: Page): Promise<boolean> {
        const passKeyErrorSelector = '[data-testid="registrationImg"]'
        const secondaryButtonSelector = 'button[data-testid="secondaryButton"]'
        ...
        try {
            ...
            const passKeyError = await page.$(passKeyErrorSelector).catch(() => null)
            ...
            if (passKeyError || isPasskeyUrl) {
                ...
                await this.ghostClick(page, secondaryButtonSelector)
                await page.waitForLoadState('networkidle', { timeout: 5000 }).catch(() => {
                    this.bot.logger.debug(this.bot.isMobile, 'PASSKEY-CHECK', 'Network idle timeout after Passkey skip')
                })
